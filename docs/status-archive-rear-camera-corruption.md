# Archive: rear camera (s5k3j1) corruption/streaming debugging saga

**Status: RESOLVED 2026-07-21.** This is the full, verbatim, chronological debugging
log for the rear camera's "black screen" / no-frames-complete streaming problem,
moved out of `STATUS.md` on 2026-07-27 to keep that file readable — nothing here
was edited or summarized, it's cut-and-pasted from the original `STATUS.md` lines
213-1640 as they stood before the archive split.

**Root cause, for anyone who doesn't need the full journal**:
`PPI2CSI_CONFIG_PPI_INTF` (register `0x204`) **bit 1**, which mainline
`ipu6-isys-csi2.c` never sets and Windows sets on every port. With it set, the
rear `S5K3J1` streams sustained 25 fps with real image data; with it clear, the
stream dies with a fatal DPHY error. Production fix: `drivers/ipu6-isys` commit
`fb9ea18`, DMI-gated bit-1 set for this exact model.

The path to that root cause runs through a lot of dead ends and false leads
(a link-frequency bug, a regulator bug, PHY trim experiments, lane-mapping
audits, live I2C captures) that are preserved below for provenance, in case a
similar-shaped bug ever shows up again on this or another IPU6 platform.
